...
*Describes build 1.1.2. Each entry records what was observed, what was decided (and what was rejected), and where the result lives in the code. For the system itself, see [ARCHITECTURE.md](ARCHITECTURE.md).*

## Rubric

**1. Industry fit judged on an operating-model axis, and cached**

- **Observed:** The lead files contain 48 distinct industry strings, including near-synonyms (SaaS / Software / Enterprise Software / Technology; Healthcare / HealthTech) that a literal lookup would score differently for no defensible reason.
...
- **Shipped:** `TIER_PROMPT` in the notebook, `factors.industry.tier_scores`, `industry_tier_cache.json`.

**2. Recency scored continuously, not in flat bands**

- **Observed:** With flat bands, a 1-day-old and a 29-day-old lead both scored 9.0, while a 31-day-old lead dropped to 6.0: a 3.0-point cliff across two days, in a quantity that decays smoothly.
- **Decided:** Interpolate linearly inside each segment, with segments meeting at their endpoints so the score has no jumps. Cutting the bands finer was rejected; it moves the cliffs rather than removing them.
...
- **Decided:** Every weight, band and cutoff was set on the 29-lead calibration file alone, then frozen before any other file was scored. The one revision (continuous recency) was made before the other files were scored, from a defect found on the calibration file. Adjusting values after seeing other files' results was rejected as fitting.
- **Shipped:** `meta.version` (`1.1-frozen-on-training`), `meta.calibration_basis`, `meta.held_out`, `meta.revision_history`.
...
- **Shipped:** `report.display.decision_order`, `build_queue()`, the `priority_rank` field.

**5. Uncertain leads go to REVIEW; missing data never causes REJECTED**
...
- **Decided:** A fit score within ±0.25 of the cutoff, any null or placeholder field, or no scoreable factors sends the lead to REVIEW. Guardrails can override a decision but never a score. Imputing missing values was rejected: zero silently disqualifies, a middle value silently inflates.
...
- **Decided:** Choose the cutoff by sweeping candidate values and comparing qualified volume with that capacity, rather than picking a round number. At 7.0, 11 of the 29 calibration leads emit QUALIFIED. Qualified volume still runs several times capacity, which is why the output is a priority queue rather than a shortlist.
...
- **Observed:** With 25 leads per call, the seven engagement-variant messages came back in one reply with 0.85 pairwise similarity — effectively one message with the names changed.
- **Decided:** Four leads per call. Similarity fell sharply, and every message landed in the 50–60 word target. The cost is 52% more tokens and more calls; that was accepted on output quality. The measurement was a single run per setting, and the decision is recorded as such.
- **Shipped:** `llm_messages.batch_size: 4`.

**9. Truncation controlled by `max_tokens`, not batch size**

- **Observed:** The model is a reasoning model, and its reasoning tokens draw on the same completion budget as the output. Two-entry calls alone ranged from 901 to 2,549 completion tokens, so entry count barely predicts demand.
...
- **Shipped:** `reconcile()`, `extract_json_objects()`, `llm_messages.content_retries`.
...
- **Decided:** Give each call its own client and parser with opposite truncation behaviour, rather than one shared function. Tiering discards the batch and its strings score as missing; messaging keeps what completed.
- **Shipped:** `call_llm_tiering()` in the notebook; `call_llm_messages()` in `message_generator.py`.
...
- **Shipped:** `TokenPacer` and `paced_message_client()` in the notebook, injected via `generate_messages(client=...)`.
...
- **Shipped:** The example in `llm_messages.variants.v3_generic` is unchanged in build 1.1.2 and does not yet meet this rule. It still implies a tool the prospect uses and runs 47 words against a 50–60 target.

**14. Leak check as a code-layer output guard**

- **Observed:** The likeliest failure in generated copy is scoring vocabulary leaking into a message. A prompt rule against it cannot guarantee compliance. On the committed sample run the check fired once, on a balanced-variant lead that then ended without a message.
- **Decided:** Check every message in code before accepting it — whole-word scoring terms plus `tier_`, `{` and `[` — and reject on a match. Relying on the prompt rule alone was rejected.
- **Shipped:** `LEAK_WORDS`, `leaked_internal()`, `message_fail_reason = leaked_internal`.

**15. Tier cache shared across files; message cache per file**

- **Observed:** The full-scale lead file contains all 29 calibration leads. With a shared message cache, its 11 qualified calibration leads would be served from disk and silently excluded from the coverage rate.
...
- **Shipped:** `industry_tier_cache.json`, `report.naming.message_cache`, `cache_key()`.

## Reporting and data

**16. Rejection reasons derived from the rubric's lowest bands**

- **Observed:** No field carried a rejection reason: `guardrails_fired` was empty on all 13 rejected calibration leads. A wider "below midpoint" threshold labelled six in-profile companies (55 to 450 employees) as "below target size".
- **Decided:** A factor earns a reason only in its lowest band or class. Several reasons on one lead are ordered by how much each one cost, so the binding constraint reads first. The vocabulary is closed: an unmapped code is surfaced in the record, never treated as a new category.
- **Shipped:** `report.reason_lookup`, `report.rejection_factor_codes`, `classify_outcome()`, `_shortfall()`.

**17. Every rate states its denominator**

- **Observed:** Two figures are true at the cutoff: 14 of 29 calibration leads score at or above 7.0, but 11 of 29 emit QUALIFIED once the review margin applies. Quoting one without saying which misstates what the system does.
...
- **Shipped:** `validate_input()`, `missing_data.sentinel_values`, `runtime.processing_date: auto`.
...
- **Blunt leak check.** It matches ordinary words such as "fit" and "priority", and it does not record which term it flagged.
- **One retry for messages.** A lead whose message fails after one content retry stays without a message.
- **Notebook only.** The tool runs only as a notebook session.
